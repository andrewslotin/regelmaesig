...
// compactUpstreamMux routes GET /stops/{id}/departures to a per-stop-ID
// handler, returning 404 for unknown IDs. This is what newTestStack /
// newCachedTestStack wrap for multi-stop tests.
func compactUpstreamMux(handlers map[string]http.HandlerFunc) http.Handler {
...
// compactUpstreamDep builds a fixture upstream departure.
func compactUpstreamDep(when, plannedWhen string, delay *float64, cancelled bool, direction, lineName, lineProduct, stopName string, remarkTypes ...string) compactUpstreamDeparture {
	d := compactUpstreamDeparture{
		When:        when,
...
func TestCompactDepartures_Success(t *testing.T) {
	base := time.Now()
	...
	deps := []compactUpstreamDeparture{
		compactUpstreamDep(t0Str, "", f64(90), false, "S+U Pankow", "U2", "subway", "S+U Alexanderplatz", "info", "warning"),
		compactUpstreamDep("", t1Str, nil, true, "Hauptbahnhof", "M4", "tram", ""),
		compactUpstreamDep(t2Str, "", f64(-90), false, "S Flughafen BER Terminal 1-2", "S9", "suburban", ""),
		compactUpstreamDep(pastWhen, "", nil, false, "Nowhere", "X1", "bus", ""), // dropped: in the past
		compactUpstreamDep("", "", nil, false, "Nowhere", "X2", "bus", ""),       // dropped: no parseable time
	}
	body, err := json.Marshal(compactUpstreamResponse{Departures: deps, RealtimeDataUpdatedAt: realtimeUpdated})
	if err != nil {
		t.Fatal(err)
	}

	upstream := compactUpstreamMux(map[string]http.HandlerFunc{
		"900100003": respondWith(http.StatusOK, string(body)),
	})
	srvURL, cleanup := newTestStack(upstream, 5*time.Second)
	defer cleanup()

	resp, err := http.Get(srvURL + "/compact/departures?stops=900100003")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompactDepartures_BadRequest(t *testing.T) {
	srvURL, cleanup := newTestStack(respondWith(http.StatusOK, `{"departures":[]}`), 5*time.Second)
	defer cleanup()

	cases := []string{
		"/compact/departures",
		"/compact/departures?stops=",
		"/compact/departures?stops=,,,",
		"/compact/departures?stops=abc",
		"/compact/departures?stops=1234567890123", // 13 digits
		"/compact/departures?stops=1,2,3,4,5",     // 5 IDs
	}
	for _, path := range cases {
		t.Run(path, func(t *testing.T) {
			...
			if string(body) != compactErrBadRequest {
...
func TestCompactDepartures_UpstreamQueryParams(t *testing.T) {
	...
	resp, err := http.Get(srvURL + "/compact/departures?stops=123&duration=999")
	if err != nil {
		t.Fatal(err)
	}
	...
	assertCompactFixedParams(t, q)

	resp, err = http.Get(srvURL + "/compact/departures?stops=123")
	if err != nil {
		t.Fatal(err)
	}
	io.ReadAll(resp.Body) //nolint:errcheck
...
func assertCompactFixedParams(t *testing.T, q url.Values) {
	t.Helper()
...
func TestCompactDepartures_Limit(t *testing.T) {
	base := time.Now()
	var deps []compactUpstreamDeparture
	for i := 0; i < 8; i++ {
		when, _ := mustRFC3339(base.Add(time.Duration(i+1) * time.Minute))
		deps = append(deps, compactUpstreamDep(when, "", nil, false, "X", fmt.Sprintf("L%d", i), "bus", ""))
	}
	body, err := json.Marshal(compactUpstreamResponse{Departures: deps})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, c := range cases {
		resp, err := http.Get(srvURL + "/compact/departures?stops=123" + c.query)
		if err != nil {
			t.Fatal(err)
		}
...
func TestCompactDepartures_StopOrderPreserved(t *testing.T) {
	base := time.Now()
	whenA, _ := mustRFC3339(base.Add(5 * time.Minute))
	whenB, _ := mustRFC3339(base.Add(5 * time.Minute))

	bodyA, err := json.Marshal(compactUpstreamResponse{Departures: []compactUpstreamDeparture{
		compactUpstreamDep(whenA, "", nil, false, "X", "A1", "bus", "Stop A"),
	}})
	if err != nil {
		t.Fatal(err)
	}
	bodyB, err := json.Marshal(compactUpstreamResponse{Departures: []compactUpstreamDeparture{
		compactUpstreamDep(whenB, "", nil, false, "X", "B1", "bus", "Stop B"),
	}})
	if err != nil {
		t.Fatal(err)
	}
	...
	resp, err := http.Get(srvURL + "/compact/departures?stops=111,222")
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close() //nolint:errcheck
...
func TestCompactDepartures_PartialFailure(t *testing.T) {
	base := time.Now()
	when, _ := mustRFC3339(base.Add(5 * time.Minute))
	bodyA, err := json.Marshal(compactUpstreamResponse{Departures: []compactUpstreamDeparture{
		compactUpstreamDep(when, "", nil, false, "X", "A1", "bus", "Stop A"),
	}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompactDepartures_TotalFailure(t *testing.T) {
	upstream := compactUpstreamMux(map[string]http.HandlerFunc{
	...
	resp, err := http.Get(srvURL + "/compact/departures?stops=111")
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close() //nolint:errcheck
	...
	body, _ := io.ReadAll(resp.Body)
	if string(body) != compactErrUpstream {
		t.Errorf("unexpected body: %s", body)
	}
}

func TestCompactDepartures_NetworkError(t *testing.T) {
...
func TestCompactDepartures_Timeout(t *testing.T) {
	upstream := compactUpstreamMux(map[string]http.HandlerFunc{
...
func TestCompactDepartures_StaleFallback(t *testing.T) {
	base := time.Now()
	when, _ := mustRFC3339(base.Add(5 * time.Minute))
	body, err := json.Marshal(compactUpstreamResponse{Departures: []compactUpstreamDeparture{
		compactUpstreamDep(when, "", nil, false, "X", "A1", "bus", "Stop A"),
	}})
	if err != nil {
		t.Fatal(err)
...
func TestCompactDepartures_PartialNotCached(t *testing.T) {
	base := time.Now()
	when, _ := mustRFC3339(base.Add(5 * time.Minute))
	bodyA, err := json.Marshal(compactUpstreamResponse{Departures: []compactUpstreamDeparture{
		compactUpstreamDep(when, "", nil, false, "X", "A1", "bus", "Stop A"),
...
func TestCompactDepartures_EmptyNotCached(t *testing.T) {
	upstream := compactUpstreamMux(map[string]http.HandlerFunc{
...
func TestCompactStripDirection(t *testing.T) {
	cases := []struct {
		in, want string
	}{
		{"S+U Pankow", "Pankow"},
	...
	for _, c := range cases {
		t.Run(c.in, func(t *testing.T) {
			if got := compactStripDirection(c.in); got != c.want {
				t.Errorf("compactStripDirection(%q) = %q, want %q", c.in, got, c.want)
			}
		})
	}
}
