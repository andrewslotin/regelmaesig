...
func TestArrivals_CacheHit(t *testing.T) {
	data := `{"arrivals":[{"when":"2099-01-01T10:00:00+01:00"}]}`
	srvURL, cleanup := newCachedTestStack(respondOnce(data), 5*time.Second, 0, 10)
	defer cleanup()

	resp, _ := http.Get(srvURL + "/stops/123/arrivals")
...
func TestArrivals_CacheMiss(t *testing.T) {
	...
	body, _ := io.ReadAll(resp.Body)
	if string(body) != emptyArrivals {
		t.Errorf("expected empty arrivals, got %s", body)
	}
}
